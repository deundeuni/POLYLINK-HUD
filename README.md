> **Multilingual Publication Notice:** This document is published in dual Korean/English versions. v1.2 2026-09-11 (Korean version: [README.ko.md](README.ko.md))  
...
## Subtitle: Zero-Modification Principle for Firmware/PLC Controllers, Base Non-invasive Reuse & Extended Fusion Hardware Combinations, Privacy by Design 4 Human Rights Protection Mechanisms & BYOD Private Space Blurring, Target Language (Native Language) Spatial Overlay for Multilingual/Multinational Sites, 1ms-class MIPI Switching & Cognitive Buffer Resource Control, Game UI/Dialogue & Venue Standard Subtitle Application Extensibility, FIRST-LIGHT/LAST-LIGHT Wide-Area Mesh L0 Anchor Lineage, 3 Sub-categories (Enterprise/Everyday/Personal BYOD), Pure Conceptual White Paper Author Zero-Liability Declaration, and soma-moa L3 Social Auxiliary Governance Defensive Publication (Ver. 1.2)

* **Official Classification:** Defensive Publication / Prior Art White Paper
* **Initial Conception Date:** 2026-08-28 / **Final Revision Date (v1.2):** 2026-09-11
...
* **Keywords:** POLYLINK-HUD, CWP-AR-HUD, Polyglot Link HUD, AR Glass HUD, Non-invasive HMI, Vision OCR, On-Device VLM, Legacy Equipment Language Barrier, Multilingual Control Panel, Target Language Agnostic, Native Language Translation, Extended Fusion, Privacy by Design, 4 Human Rights Protection Mechanisms, PIPA, EU AI Act Compliance, World-Locked Anchoring, Cybersickness Mitigation, Cognitive Read Window, Non-contact Wire Break Detection, 1ms-class Visual Response, MIPI High-Z, Overload Resource Control, Downtime Loss Mitigation, Hands-Free Maintenance, BYOD Personal AR Glasses, Game UI Subtitles, Venue Standard Subtitle Application, FIRST-LIGHT, LAST-LIGHT, Seamless Transition, First Light Last Light Link, Auxiliary Governance, Axiom 0, Author Zero Liability, soma-moa L3 Social, Tri-State Isolation, deundeuni, Prior Art

---

## 0. Author Declaration & Motivation

### 0.1 Field-Driven Motivation, Naming Philosophy & Auxiliary Governance
...
This serves as a practical implementation of `soma-moa` Charter 0 (Axiom 0): "Robots and AI are Subordinate, System Governance is Main, but even Governance must remain Auxiliary to primary human tasks." It maintains original equipment safety while maximizing first-line response capabilities of field technicians.

Identical language barriers extend beyond industrial sites into daily life (menus, signboards, textbooks, imported appliance manuals). This framework aims to mitigate entry barriers when playing console or PC games where dialogues and UIs are provided only in English, or when attending live theater, plays, and operas where the absence of subtitles hinders comprehension, forcing reliance solely on native-translated performances.
...
### 0.3 Master Concept, Base Model & Extended Fusion Hardware Combinations
...
The Base model of POLYLINK-HUD operates using standard built-in sensors (RGB camera and 6DoF IMU) on AR glasses and edge terminals, adhering to the zero additional hardware principle to prevent cost escalation. Extended Fusion configurations that combine external non-contact sensors (IR/thermal cameras, non-contact EMF sensors, microphone arrays, smart rings, haptic modules, BLE/Wi-Fi/UWB modules, edge NPUs) use this base concept as a reference point and fall completely within the scope of this prior art. By securing flexibility in hardware coupling, it proactively aims to maintain computational stability and processor overload protection even when modules are combined.
...
During network disconnections or edge processing overloads, the system maintains offline independent operation via minimal OCR functionality and cached alarm code libraries, ensuring zero operational delays or stoppages to equipment.
...
### 0.7 Universal Scope & 3 Sub-categories
...
* **Enterprise Sub-category —** Focuses on operational continuity via L0 0.1ms E-Stop, L1 Many as One, and L2 physical isolation hardware execution, applying zero-modification principles to legacy machine tools, foreign meters, and imported control panels.
* **Everyday Sub-category —** Automatically suppresses speculative output when Vibe Search inference confidence falls below 90%. Covers menus, signs, books, manuals, video call captions, console/PC game UI/dialogue, and live theater/play/opera subtitles. Restricted to temporary personal viewing; excludes recording, distribution, or derivative works.
...
* **v1.2 (2026-09-11) —** **[POLYLINK-HUD v1.2 Integrated Revision]** Removed specific corporate brand names in favor of generic terminology. Aligned author handle to `deundeuni`. Formulated everyday/entertainment extensibility (console/PC game UI/dialogue subtitles, venue standard subtitle application). Strengthened BYOD private space blurring and non-transmission protections. Defined version-agnostic citation statement for referenced specifications.
...
* **[L2] Auxiliary HMI & User Interface Layer**
  ...
  * Monitoring & History Management — Stores maintenance logs, pinout diagrams, and action guides in offline cache.
* **[L1] Perception & Inference Fabric**
  ...
  * On-Device Lightweight LLM Translation Engine — Translates domain-specific machine tool and alarm codes into target languages in real time.
...
* **[L0] Equipment & Hardware Layer**
  ...
  * [Base Model] AR glass built-in RGB camera, 6DoF IMU, microphone array, see-through display.
  * [Extended Fusion] Detachable EMF/thermal sensors, auxiliary emergency power, edge NPU processing unit.

---

## 2.5 AI Governance & Privacy by Design (4 Human Rights Protection Mechanisms)

* **Abstract AI Engine Definition —** The AI processing layer encompasses on-device edge AI, lightweight VLMs, sLLMs, server/cloud-linked models, and hybrid configurations as an abstract cognitive/inference entity.
* **Privacy by Design 4 Human Rights Protection Mechanisms —**
  * Systems are designed with human rights and privacy as top priorities, excluding eye-tracking, face analysis, or emotional evaluation functions in compliance with PIPA, EU AI Act, and applicable privacy laws.
  ...
  * **3. Technician Dignity Protection —** Ensures AI functions strictly as an auxiliary helper rather than a worker surveillance tool.
  * **4. Just Culture & Swiss Cheese Model —** Avoids penalizing worker errors, actively establishing overload mitigation buffers.
* **AI Estimation Roles —**
  * Pixel Stream Segmentation & OCR — Extracts foreign text despite screen reflections, CRT flicker, or damaged characters.
  * Industrial Context Translation — Translates complex error codes into field-accurate native terminology rather than literal translations.
  * Autonomous Hallucination Suppression — Suppresses translation output when OCR confidence drops, mitigating maintenance errors.
...
### B. Edge LLM Contextual Translation & Spatial Overlay Alignment
* Matches extracted foreign text against industrial alarm libraries to convert it into the user-configured target language in real time.
* Synchronizes control panel UI coordinates with AR glass display projection to overlay target language subtitles at 1ms-class scales.

### C. World-Locked Anchoring & Cybersickness Mitigation
* Avoids head-locked display modes, utilizing 6DoF IMU data to lock subtitles onto physical 3D control panel coordinates (World-Locked).
* Integrates with human Vestibulo-Ocular Reflex (VOR) mechanisms to eliminate visual-vestibular conflict during head movement.
* Applies deadband filtering and static latching to prevent text swimming caused by minor head tremors or sensor noise.

### D. Cognitive Read-Time Window 2-Stage Resource Control
* **Stage 1 (Initial Capture & VLM Inference) —** Triggers NPU execution at 1ms-class low latency upon detecting new error text to generate target subtitles.
...
* Prevents processor overheating, processing bottlenecks, and rendering jitter during continuous operation.
...
* Operates as an external visual/auditory/haptic auxiliary layer without interfering with primary machine control systems, mitigating firmware bricking risks.

### H. Downtime Loss Mitigation & Self-First-Action Guide
* Guides field technicians through immediate first-line resolution of simple alarms and wire breaks, dramatically reducing line stoppages caused by waiting for external service engineers.
...
* **Rate Limiter —** Normalizes camera input frame rates and OCR processing cycles to prevent processor overload and frame flooding.
* **T-Reg Suppressor —** Hardware-throttles execution cycles when terminal power or thermal metrics exceed threshold limits, mitigating device crashes during extended wear.
* **Tri-State Isolation —** Triggers a 1ms-class (0.1ms to 10ms variable range) High-Z state at the MIPI switch stage during compute errors or NPU overloads, completing display blackout within 1 frame (< 16.6ms) to prevent blocking the technician's view of physical equipment.
...
Applies to legacy CNC AR maintenance, foreign meter retrofits, everyday consumer AR guides (signs, menus, manuals), console/PC game UI/dialogue subtitles, live theater/play/opera subtitles, and underground-industry-outdoor wide-area seamless transition links. Extensible to a Venue Standard Subtitle Application Platform where personal AR glasses receive and display real-time native subtitles based on venue spatial anchors and public subtitle feeds. First inventor prior art rights for this extension belong solely to the author (`deundeuni` / `soma-moa`).

---

## 7. Practical Protection, Brand Rights & Legal Framework

* **Author authoritative Original Notice —** Legal and technical interpretation relies primarily on the Korean original (`README.ko.md`). The English version serves solely as an auxiliary reference.
...
* **Broad Scope Encompassment —** Prior art protections broadly encompass non-invasive frame capture, OCR/VLM overlays, target language independence, native language conversion, Base model sensors and Extended Fusion, Privacy by Design human rights protections, 3 sub-categories (Enterprise/Everyday/Personal BYOD), World-Locked anchoring, Cognitive Read Window resource control, non-contact wire break detection, 1ms-class MIPI switching, FIRST-LIGHT/LAST-LIGHT links, soma-moa L3 Social integration, Tri-State Isolation, game UI subtitles, venue standard subtitle applications, venue spatial anchor multi-subtitle distribution, and BYOD personal AR glasses.
...
* **White Paper Nature & Auxiliary Notice —** Disclosed technologies (POLYLINK-HUD v1.2) constitute a pure conceptual white paper published to prevent private patent monopolization. It provides no operational guarantees for commercial production and functions strictly as an auxiliary reference.
